...
class FacialRecognition
{
    ...
    private static Mat detectFaces(Mat image, CascadeClassifier faceDetector, ImageFrame frame)
    {
        ...
        for (Rect face : faces)
        {
            ...
            Imgproc.putText(image, "Pig: " + identifyFace(croppedImage), face.tl(), Font.BOLD, 3, color);
        ...
        String message = faceCount + " Pig" + (faceCount == 1 ? "" : "s") + " detected!";
    ...
    //save image as a file in png format
    //if the destination doesn't exist, create one
    private static void saveImage(Mat image, String name)
    {
        ...
        {
            int index = 0;
            //basename if the name for the separte squares to indicate faces
            //do //we tested that this do-while loop is unnecessary
